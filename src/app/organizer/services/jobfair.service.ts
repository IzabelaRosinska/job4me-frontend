...
export class JobfairService {
  ...
  getJobFairById(id: number): Observable<JobFair> {
    const route = environment.BACKEND_ROUTE + '/job-fairs/' + id;
  ...
  getJobFairByIdForList(id: number): Observable<ForListBackend> {
    const route = environment.BACKEND_ROUTE + '/job-fairs/'+id+'/job-offers/list-display';
  ...
  getJobFairs(): Observable<JobFair> {
    const route = environment.BACKEND_ROUTE + '/job-fairs';
    return this.http.get<JobFair>(route, {
      withCredentials: true,
    });
  }
  ...
  postJobFair(jobfair: JobFair): Observable<any> {
    const route =  environment.BACKEND_ROUTE +'/organizer/job-fairs';
  ...
  putJobFair(jobfair: JobFair): Observable<any> {
    const route =  environment.BACKEND_ROUTE +'/organizer/job-fairs/' + jobfair.id;
  ...
  participationEmployers$ = (jobFairId: number, page: number = 0, size: number = 5): Observable<Page<ForListBackend>> =>
      this.http.get<Page<ForListBackend>>(`${environment.BACKEND_ROUTE}/job-fairs/${jobFairId}/employers?&page=${page}&size=${size}`).pipe(shareReplay(1));
