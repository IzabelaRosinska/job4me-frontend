...
export class OrganizerService {
  ...
  getOrganizerById(id: number | string, role: string): Observable<OrganizerAccount> {
    const route = environment.BACKEND_ROUTE+'/'+role+'/organizer/'+id+'/account';
    return this.http.get<OrganizerAccount>(route, {
      withCredentials: true,
    });
  }
  ...
  jobfairsOrganizer$ = (status: boolean = true, page: number = 0, size: number = 5): Observable<Page<JobFair>> =>
      this.http.get<Page<JobFair>>(`${environment.BACKEND_ROUTE}/organizer/employer-participation?&page=${page}&size=${size}&status=${status}`).pipe(shareReplay(1));


  acceptEmployerParticipation(requestId: number): Observable<any> {
      const route =  environment.BACKEND_ROUTE +'/organizer/employer-participation/'+requestId+'/accept';
  ...
  deleteEmployerParticipation(requestId: number): Observable<any> {
    const route =  environment.BACKEND_ROUTE +'/organizer/employer-participation/'+requestId;
